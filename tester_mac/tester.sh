...
while [ $i -le 100 ]
do
	ARG=`ruby -e "puts (0..$INDEX).to_a.shuffle.join(' ')"`; echo $ARG >> output.txt ; ../build/bin/push_swap $ARG | ./checkers/checker_Mac $ARG >> output.txt; ../build/bin/push_swap $ARG | wc -l >> output.txt
  ((i++))
done
